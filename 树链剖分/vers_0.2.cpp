...
inline void update(int i, int l, int r, int k)
{
	if (tree[i].l >= l && tree[i].r <= r) { 
		tree[i].sum += (tree[i].r - tree[i].l + 1) * k;  tree[i].sum %= mod; tree[i].lz += k; tree[i].lz %= mod; return; 
	}
	push_down(i);
...
inline void change(int x, int y,int k)
{
	while (top[x] != top[y])
	{
		if (dep[x] < dep[y]) { swap(x, y); }
 		update(1, dfsk[top[x]], dfsk[x], k);
		x = fa[top[x]];
	}
	if (dep[x] > dep[y]) { swap(x, y); }
	update(1, dfsk[x], dfsk[y], k);
}
inline int  ask(int x, int y)
{
	...
	{
		if (dep[x] < dep[y]) { swap(x, y); }
		res += query(1, dfsk[top[x]], dfsk[x]);
		res %= mod;
		x = fa[top[x]];
	}
	if(dep[x] > dep[y]) { swap(x, y); }
	res += query(1, dfsk[x], dfsk[y]);
	res %= mod;
	return res;
}
int main()
{
	...
	while (m--)
	{
		...
		case 3:
			cin >> x >> k;
			update(1,dfsk[x], dfsk[x] + siz[x] - 1, k);
			break;
		case 4:
			cin >> x;
			cout << query(1, dfsk[x], dfsk[x] + siz[x] - 1)<<endl;
			break;
		}
	}
	return 0;
